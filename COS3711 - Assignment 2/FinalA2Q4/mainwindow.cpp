...
void MainWindow::ui() {
    ...
    queryLayout->addWidget(new QLabel("Query by Name:"));
    ...
    affiliationLayout->addWidget(new QLabel("Query by Affiliation:"));
    ...
    buttonLayout->addWidget(loadButton); // Added load button to layout
...
void MainWindow::registerAttendee() {
    ...
    Registration *registration = RegistrationFactory::getInstance().makeRegistration(type, attendee, qualification, category);

    if (!registration) {
        QMessageBox::warning(this, "Registration Error", "Invalid registration type.");
        delete attendee;
        return;
    }
...
void MainWindow::loadRegistrationList() {
    ...
    if (reader.readFile(fileName, registrationList)) {
        table();
